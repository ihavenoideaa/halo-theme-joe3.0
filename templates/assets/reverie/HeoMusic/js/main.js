...
var local = false;
const resource_link = "/themes/theme-Joe3/assets/reverie/HeoMusic";
...
function loadMeting() {
  ...
  script.src = `${resource_link}/js/Meting.js`;
...
function loadLocalEngine() {
  removeScript(script);
  script = document.createElement('script');
  script.src = `${resource_link}/js/localEngine.js`;
  document.body.appendChild(script);
  local = true;
}
...
var heo = {
  ...
  setMediaMetadata: function (aplayerObj, isSongPlaying) {
    ...
    const coverUrl = audio.cover || `${resource_link}/img/icon.webp`;
    //const currentLrcContent = document.getElementById("heoMusic-page").querySelector(".aplayer-lrc-current").textContent;
    const heoMusicPage = document.getElementById("heoMusic-page");
    const currentLrcElement = heoMusicPage ? heoMusicPage.querySelector(".aplayer-lrc-current") : null;
  ...
  updateThemeColorWithImage(img) {
    if (local) {
      ...
      if (typeof ColorThief === 'undefined') {
        ...
        script.src = `${resource_link}/js/color-thief.min.js`;
